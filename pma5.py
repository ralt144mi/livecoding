...
def liquide(d=0.5,i=0):
    ...
    M(note = "C@min7", channel = 0, dur=400).out(i, div = 2)
    M(note=".!7,<C@min7>,.!7,<C@min7>,.!7,<C@maj7>",
            channel = 0, dur=300).out(i, div=4)
    #LeadTHEME  #apal #^[2~6]
    #M(note="apal(<C@maj, C@min7>,68,65,.,67,.)^[2~6]" , 
    #    velocity = P("70~90",i), 
...
PO >> play_midi(note="36,.,.,.", velocity = 127, channel = 9)
...
PO >> play_midi(note="36,.,36,36,.,36,36,36",
        velocity = 130, channel =9)

PO >> None

Ps >>  play_midi(note = ".,38,.,38", velocity = 85,  channel =9)
